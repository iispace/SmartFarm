...
### 4S-SIF의 가능성 (분광복사계의 대안)

- 4S-SIF는 SIF 측정 분야의 높은 장벽을 극복할 수 있는 잠재력이 있음.
- 고해상도 분광복사계와 같은 기존 SIF 관측 장비는 다양한 파장(O₂-A, O₂-B 등)과 추출 방법을 비교할 수 있지만, 온도.습도 제어, 복잡한 보정 작업 등 유지 관리가 어렵고 비용 또한 고가임.
- 4S-SIF 센서는 포토다이오드와 bandpass filter의 조합으로 구성되어
  - 간단한 수식만으로 SIF나 식생 지수(NDVI, PRI 등)를 계산할 수 있음.
  - 사용이 쉬우며, 분광학 지식이 없이도 운용 가능
  - 상대적으로 저렴한 가격으로 접근성이 높아짐.
  - 4S-SIF 센서의 구조(포토다이오드 + 필터)와 유사한 상용 센서들도 있지만, 4S-SIF는 필터 교체를 통해 원하는 파장대의 빛을 관측할 수 있는 확장성이 있음.

- 최근 연구에서는 SIF 관측 방식의 차이점이 측정 결과에 중요한 영향을 줄 수 있음을 강조함.
- 대표적인 관측 방식:
