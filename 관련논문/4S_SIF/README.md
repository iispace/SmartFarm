...
- SIF의 원격 탐사 메커니즘을 더 잘 이해하고 설명하기 위해서는 다양한 지역과 생물군계(biomes)에서 지상 기반 SIF 관측을 수행하는 것이 필요함.
- 이를 위해 연구자들은 식물 캐노피 수준의 SIF를 지속적으로 모니터링할 수 있는 4S-SIF 센서를 개발함.
- 4S-SIF는 초협대역 대역통과 필터와 포토다이오드를 결합한 구조임.
- 4S-SIF는 기준 분광복사계와 비교했을 때 빛의 세기에 대한 선형적 응답 특성을 보였으며, SIF 추출에 사용된 모든 파장대에서 입사광에 대한 코사인 응답 곡선도 거의 최적에 가까운 형태임이 확인됨.
- 또한, 초협대역 대역통과 필터의 온도 반응 특성을 측정하고, 시뮬레이션을 기반으로 이를 보정함.
- 4S-SIF와 기준 분광복사계를 딸기 재배지에 설치하여 개발된 센서 시스템이 SIF 신호를 감지할 수 있는지 확인.
- 제초제 처리 전후에 4S-SIF로 추출한 SIF 값은 기준 센서로부터 얻은 SIF 값과 잘 일치하는 결과를 보였음.
- 4S-SIF가 다양한 공간적·시간적 규모에서 현장(in-situ) 데이터를 수집하는 데 유용한 도구가 될 것이라 믿으며, 이 센서를 개발하면서 얻은 경험은 유사한 센서를 설계하려는 연구자들에게 도움이 될 것으로 기대함.
